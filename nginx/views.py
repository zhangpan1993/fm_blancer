...
from subprocess import check_output,CalledProcessError,getstatusoutput
from jinja2 import Environment,FileSystemLoader
from django.conf import settings
from proxy.models import upstream_config,proxy_config
...
def reload_config():
    ...
    for p in proxy_config_list:
        ...
        config_proxy_path = "/etc/nginx/conf/%s.conf"%p.config_id
        if p.protocols:
...
def get_sys_status():
    ...
    try:
        nginx_status = bool(len(list(map(int,check_output(['pidof','nginx']).split()))))
