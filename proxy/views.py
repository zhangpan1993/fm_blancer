...
def view(request):
    ...
    return render_to_response('proxy/view.html',{'proxy':contexts,
                                                 'filter':filter,
                                                 'user':user,
                                                 })
    ...
    try:
        ...
        if nic:
            if proxy_name and server_name and listen and len(post['upstream_list']) and not listen=="8000":
                ...
                config_path = "/etc/nginx/%s.conf" %config_id
                ...
                if 'proxy_ip_hash' in post['base_config']:
                    balancer_type = "ip_hash"

                if 'proxy_http_check' in post['base_config']:
                    check_type = "http"
                else:
                    check_type = "tcp"
                ...
                if 'ssl_status' in post['ssl_config']:
                    ...
                    if cert_body and key_body:
                        ...
                        if 'ssl_port' not in post['ssl_config']:
                            proxy['listen'] = 443
                else:
                    if proxy['listen'] == 443:
                        proxy['listen'] = 80
