...
def save(request):
    ...
    try:
        ...
        if "auto_worker_processes" in post:
            ...
            worker_processes = post.get("worker_connections").replace("_","")
        ...
        if worker_processes or worker_connections and keepalive_timeout and client_max_body_size:
            config_id = str(uuid.uuid1())
            config_path = "/etc/nginx/conf/nginx.conf"
